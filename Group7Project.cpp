...
void BBinsertion(int a[], int s) //This is the insertion Sort where the string of the input from the main will go in these void parameters. As insertion sort is to move the elements only one ahead of its position.
{
	...
	{
		for (int j = i; j > 0; j--) //where it will compare the next letters to the preivous letter
		{
            if(a[j] < a[j-1])
            {
                int temp; //temporary
			...
			{
				break; //if the letter is not less than the previous letter it breaks out the loop
			}
		}
		
	}
	cout << a << endl; //outputing the letters
}
...
{
    int B[10]; //Placing the B and initalize it here which would be 10 as that is how many value is being sorted.
    int C[10] = { 0 }; //Where I was stucked for a bit where I had to initalize the C[10] to be 0. That is one of the difference for this Radix sort to work compare to just CountingSort. Took me an hour even with breakpoints as it wasn't picking it up as the problem. 
